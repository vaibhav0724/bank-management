...
void insert()
{
    ...
    if (fp1 == NULL)
    {
        printf("Sorry, Technical fault, Try later!\n");
        return;
    }
    struct Customer_details Customer; // Declare a local instance of Customer_details
...
void search()
{
    char sname[4]; // Increase the size to match the size of _grp
    FILE *fp;
    int flag = 0;
    fp = fopen("details.dat", "r");
    if (fp == NULL)
